...
export default function ParamPanel({ caseId }: ParamPanelProps) {
  ...
  // Debounced slider-move counter for the per-case scoring (Phase A).
  // 400 ms quiet window so a single slider scrub = 1 move, not 47.
  const moveTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const isInitialApply = useRef(true);
  const countSliderMove = useStore((s) => s.countSliderMove);

  useEffect(() => {
    if (!caseDef) return;
    isInitialApply.current = true;
    const sub = watch((values) => {
      ...
      // Skip the very first watch fire (it's RHF flushing the default values
      // right after reset, not a real user interaction).
  ...
  }, [watch, caseDef, setParams, countSliderMove]);
