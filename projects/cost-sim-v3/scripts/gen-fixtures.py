...
def main():
    ...
    # Case 7 (Boss): Loading + Labor + Yield 3-way crisis (Phase C)
    # Composes the existing 3 transforms in sequence to produce a single
    # combined scenario. The TS engine reproduces the exact same sequence.
    sim7 = apply_loading_change(REFERENCE_CASE1, 0.60)
    sim7 = apply_labor_change(sim7, 1.30)
    sim7 = apply_material_yield_change(sim7, 0.0, -0.02)
    scenarios["case7_crisis_combo"] = {
        "description": "Q3 위기 — Loading 60% + 임금 1.3x + 수율 -2%p (Boss)",
        "adapter": "crisis",
        "input": {"new_loading": 0.60, "labor_multiplier": 1.30, "module_yield_change": -0.02},
        "params_after": params_to_dict(sim7),
        "result": result_to_dict(calculate(sim7)),
    }
